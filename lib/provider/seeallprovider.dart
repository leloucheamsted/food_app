import 'package:slike/model/rentsectiondetailmodel.dart' as rent;
import 'package:slike/model/rentsectiondetailmodel.dart';
import 'package:slike/model/sectiondetailmodel.dart' as section;
...
class SeeAllProvider extends ChangeNotifier {
      ...
      if (sectionListModel.result != null &&
        ...
        if (sectionListModel.result != null &&
          ...
          for (var i = 0; i < (sectionListModel.result?.length ?? 0); i++) {
            sectionDataList?.add(
              sectionListModel.result?[i] ?? section.Result(),
            );
          }
  ...
  Future<void> getRentSeactionDetail(sectionId, pageNo) async {
    loading = true;
    rentSectionDetailModel = await ApiService().rentSectionDetail(
      sectionId,
      pageNo,
    );
      ...
      if (rentSectionDetailModel.result != null &&
        ...
        if (rentSectionDetailModel.result != null &&
          ...
          for (
            ...
            rentVideoList?.add(
              rentSectionDetailModel.result?[i] ?? rent.Result(),
            );
          }
