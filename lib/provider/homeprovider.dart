...
import 'package:slike/model/getcontentbychannelmodel.dart' as playlist;
...
import 'package:slike/model/categorymodel.dart' as cat;
import 'package:slike/model/videolistmodel.dart' as video;
...
class HomeProvider extends ChangeNotifier {
  ...
  Future<void> getvideolist(ishomePage, categoryid, pageNo) async {
    videoloading = true;
    videolistmodel = await ApiService().videolist(
      ishomePage,
      categoryid,
      pageNo,
    );
  ...
  setVideoListPaginationData(
    int? videolisttotalRows,
    int? videolisttotalPage,
    int? videolistcurrentPage,
    bool? videolistisMorePage,
  ) {
    ...
    if (getContentbyChannelModel.status == 200) {
      ...
      if (getContentbyChannelModel.result != null &&
        ...
        for (
          ...
          playlistData?.add(
            getContentbyChannelModel.result?[i] ?? playlist.Result(),
          );
        }
  ...
  getcreatePlayList(chennelId, title, playlistType) async {
    loading = true;
    createPlaylistModel = await ApiService().createPlayList(
      chennelId,
      title,
      playlistType,
    );
