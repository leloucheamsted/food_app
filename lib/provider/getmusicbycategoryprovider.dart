...
import 'package:slike/model/getmusicbycategorymodel.dart' as music;
...
class GetMusicByCategoryProvider extends ChangeNotifier {
  ...
  Future<void> getMusicbyCategory(categoryId, pageNo) async {
    loading = true;
    getMusicByCategoryModel = await ApiService().getMusicbyCategory(
      categoryId,
      pageNo,
    );
      ...
      if (getMusicByCategoryModel.result != null &&
        ...
        if (getMusicByCategoryModel.result != null &&
          ...
          for (
            ...
            musicList?.add(
              getMusicByCategoryModel.result?[i] ?? music.Result(),
            );
          }
