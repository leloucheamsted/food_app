...
import 'package:slike/model/getmusicbylanguagemodel.dart' as music;
...
class GetMusicByLanguageProvider extends ChangeNotifier {
  ...
  Future<void> getMusicbyLanguage(languageId, pageNo) async {
    loading = true;
    getMusicByLanguageModel = await ApiService().getMusicbyLanguage(
      languageId,
      pageNo,
    );
      ...
      if (getMusicByLanguageModel.result != null &&
        ...
        if (getMusicByLanguageModel.result != null &&
          ...
          for (
            ...
            musicList?.add(
              getMusicByLanguageModel.result?[i] ?? music.Result(),
            );
          }
